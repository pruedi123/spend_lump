...
def load_factors(path: str) -> pd.DataFrame:
    ...
    def _k(n): 
        try: return int(str(n).replace("E",""))
        except: return 999
...
def lookup_withdrawal(df_w: pd.DataFrame|None, yrs: int):
    ...
    try: return float(df_w.loc[df_w["Years"] == yrs, "Median"].iloc[0])
...
fee_mult_glob = (1.0 - 0.0020) ** (12/12)
fee_mult_spx  = (1.0 - 0.0005) ** (12/12)
...
def _best(col_med, col_min):
    if raw_df.empty: return 0.0, 0.0
...
if rows2:
    ...
    fmt["Total Median Retirement Income"] = fmt["Total Median Retirement Income"].map(lambda v: f"${v:,.0f}")
