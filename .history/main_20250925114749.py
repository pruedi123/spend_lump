...
if w_g is not None and np.isfinite(g_min_ev):
    ...
    rows2_min.append({
        ...
        "Minimum Ending Value": g_min_ev,
        "Annual Retirement Income (Historically)": ann_g_min,
...
if w_s is not None and np.isfinite(s_min_ev):
    ...
    rows2_min.append({
        "Portfolio": "SPX",
        "Years": retirement_years,
        "Minimum Ending Value": s_min_ev,
        "Annual Retirement Income (Historically)": ann_s_min,
        "Total Minimum Retirement Income": ann_s_min * retirement_years,
    })

if rows2_min:
    st.subheader("Minimum Withdrawal — Lump Sum")
    ...
    fmt_min = fmt_min[[
        "Portfolio",
        "Years",
        "Minimum Ending Value",
        "Annual Retirement Income (Historically)",
        "Total Minimum Lifetime Retirement Income",
    ]]
    st.dataframe(fmt_min, use_container_width=True)